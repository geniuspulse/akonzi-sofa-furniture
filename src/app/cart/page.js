...
function CartContent() {
  ...
  const handleWhatsAppCheckout = () => {
    ...
    let msg = `Hello Akonzi Furniture! I'd like to order:\n\n`;
    cart.forEach(item => {
    ...
    const url = `https://wa.me/265000000000?text=${encodeURIComponent(msg)}`;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '32px', alignItems: 'start' }}>
          ...
          <div className="cart-summary">
            ...
            <div className="cart-actions" style={{ flexDirection: 'column' }}>
              ...
              <button onClick={clearCart} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '0.9rem', textDecoration: 'underline' }}>Clear Cart</button>
            </div>
            <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '16px', lineHeight: '1.5' }}>
              Clicking "Checkout via WhatsApp" will open a pre-filled WhatsApp message with your order details.
            </p>
          </div>
        </div>
      </div>
